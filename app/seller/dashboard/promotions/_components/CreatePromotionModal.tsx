...
export function CreatePromotionModal({ open, onOpenChange, onSuccess }: CreatePromotionModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success('Tạo khuyến mãi thành công, chờ quản trị viên duyệt')
      onOpenChange(false)
      onSuccess()
      setFormData({
        name: '',
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Điền thông tin khuyến mãi bên dưới. Khuyến mãi sẽ được gửi để quản trị viên duyệt.
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) =>
                  handleChange('priceOrderActive', e.target.value ? parseFloat(e.target.value) : null)
                }
                className="mt-1"
              />
            </div>
