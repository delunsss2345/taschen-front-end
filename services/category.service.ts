...
export const categoryService = {
  async getAllCategories(): Promise<Category[]> {
    try {
      const response =
        await http.get<ApiResponseEnvelope<Category[]>>("categories");
  ...
  async createCategory(payload: CreateCategoryRequest): Promise<Category> {
    const response = await http.post<ApiResponseEnvelope<Category>>(
      "categories",
      payload,
    );
    return requireResponseData(
      response,
      "Create category response is missing data",
    );
  },
  ...
  async updateCategory(
    ...
    const response = await http.put<ApiResponseEnvelope<Category>>(
      `categories/${categoryId}`,
      payload,
    );
    return requireResponseData(
      response,
      "Update category response is missing data",
    );
  },

  async deleteCategory(categoryId: number | string): Promise<void> {
    await http.del<ApiResponseEnvelope<null>>(`categories/${categoryId}`);
  },
};
