...
export const categoryService = {
  ...
  async getCategoryById(categoryId: number | string): Promise<Category> {
    const response = await http.get<ApiResponseEnvelope<Category>>(`categories/${categoryId}`);
  ...
  async createCategory(payload: CreateCategoryRequest): Promise<Category> {
    const response = await http.post<ApiResponseEnvelope<Category>>("categories", payload);
  ...
  async updateCategory(
    ...
    const response = await http.put<ApiResponseEnvelope<Category>>(`categories/${categoryId}`, payload);
